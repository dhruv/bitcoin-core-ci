...
void DeriveBIP324Session(ECDHSecret&& ecdh_secret, BIP324Session& session)
{
    ...
    hkdf.Expand32("garbage_terminator", hkdf_32_okm);
    memcpy(session.garbage_terminator.data(), hkdf_32_okm, BIP324_GARBAGE_TERMINATOR_LEN);
                ...
                if ((pnode->m_prefer_p2p_v2 && pnode->IsInboundConn() && !pnode->v2_garbage_terminated) ||
                    ...
                    if (gArgs.GetBoolArg("-v2transport", DEFAULT_V2_TRANSPORT) && !pnode->tried_v2_handshake) {
                        ...
                        if (pnode->IsInboundConn() && !pnode->v2_garbage_terminated && num_bytes > 0) {
                            ...
                            // TODO: Is it better to just allocate to V2_MAX_GARBAGE_BYTES once?

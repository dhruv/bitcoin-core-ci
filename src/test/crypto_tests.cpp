...
static void TestFSChaCha20(const std::string& hex_plaintext, const std::string& hexkey, size_t rekey_interval, const std::string& ciphertext_after_rotation)
{
    ...
    std::memcpy(key.data(), key_vec.data(), FSCHACHA20_KEYLEN);
    ...
    for (size_t i = 0; i < rekey_interval; i++) {
        ...
        BOOST_CHECK_EQUAL(0, std::memcmp(c20_output.data(), fsc20_output.data(), plaintext.size()));
    }
    ...
    auto c20_copy = c20;
    c20.Crypt(plaintext.data(), c20_output.data(), plaintext.size());
    BOOST_CHECK(std::memcmp(c20_output.data(), fsc20_output.data(), plaintext.size()) != 0);
...
static void TestBIP324CipherSuite(const std::string& hex_aad, const std::string& hex_contents, const std::string& hex_key_L, const std::string& hex_key_P, const std::string& hex_expected_output_seq_0, const std::string& hex_expected_output_seq_999)
{
    ...
    BIP324Key key_L;
    std::memcpy(key_L.data(), key_L_vec.data(), BIP324_KEY_LEN);

    auto key_P_vec = ParseHex(hex_key_P);
    BIP324Key key_P;
    std::memcpy(key_P.data(), key_P_vec.data(), BIP324_KEY_LEN);
    ...
    for (size_t i = 0; i < 1000; ++i) {
        ...
        std::memcpy(encrypted_pkt_len.data(), encrypted_pkt.data(), BIP324_LENGTH_FIELD_LEN);
        ...
        if (!original_contents_bytes.empty()) {
            BOOST_CHECK_EQUAL(0, std::memcmp(contents_buf_dec.data(), original_contents_bytes.data(), original_contents_bytes.size()));
        }
    }
}
...
{
    auto aad = ParseHex(hex_aad);
    auto key_vec = ParseHex(hex_key);
    RFC8439Key key;
    std::memcpy(key.data(), key_vec.data(), RFC8439_KEYLEN);
    auto nonce = ParseHex(hex_nonce);
    std::array<std::byte, 12> nonce_arr;
    std::memcpy(nonce_arr.data(), nonce.data(), 12);
    auto plaintext = ParseHex(hex_plaintext);
    std::vector<std::byte> output(plaintext.size() + POLY1305_TAGLEN, std::byte{0x00});
    RFC8439Encrypt(MakeByteSpan(aad), key, nonce_arr, MakeByteSpan(plaintext), output);
    ...
    auto authenticated = RFC8439Decrypt(MakeByteSpan(aad), key, nonce_arr, output, decrypted_plaintext);
    BOOST_CHECK(authenticated);
    BOOST_CHECK_EQUAL(0, std::memcmp(decrypted_plaintext.data(), plaintext.data(), plaintext.size()));
}

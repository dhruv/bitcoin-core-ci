...
FUZZ_TARGET(crypto_rfc8439)
{
    ...
    auto key_vec = fdp.ConsumeBytes<std::byte>(RFC8439_KEYLEN);
    key_vec.resize(RFC8439_KEYLEN);
    RFC8439Key key;
    ...
    std::memcpy(nonce.data(), nonce_vec.data(), 12);
    ...
    if (bit_flip_attack) {
        ...
        assert(authenticated);
        if (!plaintext.empty()) {
            assert(std::memcmp(decrypted_plaintext.data(), plaintext.data(), plaintext.size()) == 0);
        }
    }
}
